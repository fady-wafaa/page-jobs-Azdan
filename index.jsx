...
const ContainerUl = ({ ArrDepar, data, arrDepar }) => {
  // console.log("ArrDepar",data.filter( e => e.departmentT == ArrDepar.map(e => e)).length)
  // console.log("ArrDepar",document.querySelector(".AllSelect"))

  const [arrdef, setArrDef] = useState(data);
        ...
        {arrDepar.length != 0 ? (
          arrDepar.length > 0 ? (
            arrDepar.map((el, index) => (
              //  {       let ele = data.filter( e => e.departmentT == el).length}
...
const Positions = ({ ArrDepar, work, location, data }) => {
  ...
  const valueSelect = (e) => {
    ...
    if (clieckNowInp == "sele1") {
        ...
        let filterDep = data.map((el) => el.departmentT === e.currentTarget.querySelector("h3").innerHTML && el.locationText == "Alex, Egypt"
            ? el
            : null
        ).filter((e) => e !== null);
    ...
    //         let elem = [...document.querySelectorAll(`.sele3 li `)][
    //           i
    //         ].querySelector("h3").innerText;
    //         data = data
    //           .map((e) => (e.locationText == elem   ? e : null))
    //           .filter((e) => e !== null);
    ...
    //                 : null
    //             )
    //             .filter((e) => e !== null && e.locationText== "Alex, Egypt");
    ...
    //     let filterDep = data
    //       .map((el) =>
    //         el.departmentT === e.currentTarget.querySelector("h3").innerHTML
    //           ? el
    //           : null
    //       )
    //       .filter((e) => e !== null);
    ...
    let check = [
      ...document.querySelectorAll(`.${trgetClass} .icon-tick`),
    ...
    if (check.length > 0) {
      if (trgetClass == "sele1") {
        ...
        if (checkLI == 100) {
          ...
          if (
            document
              .querySelector(`.${trgetClass} ul li h3`)
              .classList.contains("font-weight-bold")
          ) {
            document
              .querySelector(`.${trgetClass} .selection__clear`)
              .classList.add("show");
